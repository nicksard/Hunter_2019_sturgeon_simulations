...
#defining path to COLONY on local machine - WILL BE SPECIFIC TO EACH COMPUTER
my.colony.path <- "C:/ZSL/Colony/SimuPolyPloid.exe"
...
colony.sim.create <- function(mat.str, update.alfs = 0, spp.type = 2, selfing.rate = 0, inbreeding = 0,
                              ...
                              convert.codom = 1, codom_dropout = 0.01, codom_false.allele = 0.02, 
                              delete.sup.files = T){
  ...
  n.mating.str <- paste(n.mating.str, "! Number of mating structures",sep = "\t")
  n.mom <- ncol(mat.str)
  n.dad <- nrow(mat.str)
  ...
  gamy <- paste(paste(mal.gamy,fem.gamy),"! 1/0=Monogamy/Polygamy for males & females",sep = "\t")
  ...
  convert.codom <- paste(convert.codom,"! 1/0=Yes/No for converting codominant to dominant loci, MUST BE 1 for polyploid",sep = "\t")
  sib.scale <- paste(sib.scale,"! 0/1=Scale full sibship=No/Yes",sep = "\t")
  codom_dropout <- paste(codom_dropout,"! Assumed dropout rate per converted marker",sep = "\t")
  codom_false.allele <- paste(codom_false.allele,"! Assumed false error rate rate per converted marker",sep = "\t")
  ...
  if(allele.dist != 3 ){
    ...
    cat(ncand,prcand, nloci, pr.miss.gt, gt.err, mut.err, marker.type, n.alleles,allele.dist, ploidy,
        gamy, random.seed, sib.prior, known.alfs, update.alfs, n.reps,
        run.length, map.length, inbrd.coef,inbreeding, windows.version, monitor.interval, 
        convert.codom, sib.scale, codom_dropout, codom_false.allele,
        file = "input3.Par",sep = "\n",append = T)
  } else {
